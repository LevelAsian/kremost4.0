...
var fs = require('fs'),
    ...
    reEscapeChars = /[.|\-[\]()\\]/g,
    reAsterisk = /\*/g,
    // Flag to distinguish an www crash from intentional killing (used on Windows only for now)
...
function startNode() {
  ...
  child.on('exit', function (code, signal) {
    // In case we killed the www ourselves, set the signal thusly
    ...
    if (signal === 'SIGUSR2') {
      ...
      util.log('\x1B[1;31m[nodemon] www crashed\x1B[0m');
      process.exit(0);
    } else {
      util.log('\x1B[1;31m[nodemon] www crashed - waiting for file changes before starting...\x1B[0m');
      child = null;
    }
  });
...
function getAppScript(program) {
  ...
  if (!program.args.length || program.app === null) {
    // try to get the www from the package.json
    ...
    try {
      ...
      if (program.app === undefined) {
        // no www found to run - so give them a tip and get the feck out
        help();
      }
...
function help() {
  util.print([
    ...
    '  -x, --exec www     execute script with "www", ie. -x "python -v"',
...
// on exception *inside* nodemon, shutdown wrapped node www
...
// process.chdir(path.dirname(www));
dirs.forEach(function(dir) {
  if (program.options.verbose) {
